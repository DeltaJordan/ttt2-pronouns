...
local function PlayerAppendPronouns(ply, panel)
    ...
    if string.len(prons) > appendMaxChar then
        prons = string.sub(prons, 1, appendMaxChar-2).."..."
    end

    prons = "("..prons..")"

	-- trigger PerformLayout which should set everything in the right position for this to also be set in the right position.
...
hook.Add("TTTScoreboardColumns", "PronounsScoreboard", function(panel)
	...
	if panel.open ~= nil then
		...
		timer.Create("UpdatePronounsNextThink", 1/150, 2, function()
	...
	if GetConVar("ttt2_pronouns_scoreboard_column"):GetBool() then
		panel:AddColumn("Pronouns", function(ply)
			...
			if string.len(prons) > columnMaxChar then
				prons = string.sub(prons, 1, columnMaxChar-2).."..."
			end
